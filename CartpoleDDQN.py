...
from src.learner.DDQN import DDQN

tf.random.set_seed(10)
np.random.seed(10)


class CartpoleV0DDQN(CarpoleV0DQN):
    def createAgent(self):
        replay_buf = MostRecentReplayBuffer(2 * self.minibatchSize)
        return DDQN(self.qfunc, self.emulator, self.nfeatures, self.nactions,
                    replay_buffer=replay_buf, discount_factor=self.discountFactor,
                    minibatch_size=20, epochs_training=20, sync_period=2)


if __name__ == "__main__":
    cartpole = CartpoleV0DDQN()
    cartpole.balance()
